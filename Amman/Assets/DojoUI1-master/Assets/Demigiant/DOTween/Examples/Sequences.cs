...
public class Sequences : MonoBehaviour
{
	...
	void Start()
	{
		...
		// Let's create cursor Sequence which will first rotate AND move the target's Y at the same time,
		...
		// Add cursor 1 second move tween only on the Y axis
		mySequence.Append(target.DOMoveY(2, 1));
		// Add cursor 1 second rotation tween, using Join so it will start when the previous one starts
		mySequence.Join(target.DORotate(new Vector3(0, 135, 0), 1));
		// Add cursor 1 second scale Y tween, using Append so it will start after the previously added tweens end
